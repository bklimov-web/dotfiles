...
`brew/`, `scripts/` и `raycast/` — не stow-пакеты, обычные папки с файлами.
Их не нужно (и не стоит) передавать в `stow`.

## Raycast

`raycast/raycast-export.rayconfig` — экспорт настроек Raycast (Settings → Advanced →
Export), включая хоткеи отдельных команд и расширений — то, что не лежит в обычном
`defaults` и не покрывается `macos-defaults.sh` (см. ниже). Файл зашифрован паролем,
которого нет в репозитории — он в 1Password. Без пароля файл бесполезен, поэтому
хранить его в публичном репо безопасно.

Обновить: Raycast → Settings → Advanced → Export → заменить файл в репозитории.
Восстановить на новой машине: Raycast → Settings → Advanced → Import → указать файл,
ввести пароль из 1Password.
...
Клавиатура (быстрый повтор, без автозамен), трекпад (tap to click), Finder
(скрытые файлы, расширения, path bar, status bar), Dock (автоскрытие без
задержки, размер иконок, без недавних приложений), Spotlight (Cmd+Space
выключен) и Raycast (сам открывается по Cmd+Space вместо него). Не входит
в `install.sh` — меняет вид рабочего стола сразу и заметно, поэтому
запускается отдельно и осознанно.

Хоткеи отдельных команд и расширений внутри Raycast сюда не входят — они
лежат в зашифрованном `raycast-enc.sqlite`, а не в обычном `defaults`.
Бэкапятся отдельно через Raycast → Settings → Advanced → Export.
